...
function UrlPill({ url, color }: { url: string; color: string }) {
  return (
    <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 12, border: `1px solid ${color}`, color, fontSize: 10, marginRight: 6, marginTop: 4, fontFamily: "'Fira Code', 'Courier New', monospace" }}>
      {"🔗 "}{url.replace(/^https?:\/\//, "")}
    </span>
  );
}
              ...
              <div key={i} style={{ marginBottom: 10, padding: "8px 12px", background: "#F8FAFC", borderRadius: 6, border: "1px solid #E2E8F0" }}>
                ...
                {p.urls?.length > 0 && (
                  <div style={{ marginTop: 4 }}>
                    {p.urls.map((u, j) => <UrlPill key={j} url={u} color={green} />)}
                  </div>
                )}
              </div>
            ))}
