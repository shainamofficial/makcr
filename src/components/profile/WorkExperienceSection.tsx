...
export default function WorkExperienceSection({ data }: { data: any[] }) {
  ...
  const handleDelete = async () => {
    if (!deleting) return;
    // Delete points first, then experience
          ...
          <Card key={we.id}>
            <CardHeader className="pb-2">
              <div className="flex items-start justify-between">
                <div>
                  <CardTitle className="text-base">{we.company?.name}</CardTitle>
                  <p className="text-sm text-muted-foreground">{we.title}</p>
            ...
            <CardContent>
              {we.description && <p className="text-sm mb-2">{we.description}</p>}
